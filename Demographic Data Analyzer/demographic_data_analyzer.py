...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = df.groupby('native-country')['salary'].agg(lambda x: (x == '>50K').mean()).idxmax()
    highest_earning_country_percentage = round(df.groupby('native-country')['salary'].agg(lambda x: (x == '>50K').mean()).max() * 100, 1)
